...
interface file {
    ...
    preferences: {
        cnpjValue: string,
        cnpjAutoFormat: boolean,
        cnpjAutoCopy: boolean,
        a7Value: string,
        a7AutoFormat: boolean,
        phoneAndEmailValue: string,
        phoneAndEmailAutoFormat: boolean,
        phoneAndEmailAutoCopy: boolean
    }
};
...
const defaultConfig: any = {
    ...
    preferences: {
        cnpjValue: '',
        cnpjAutoFormat: false,
        cnpjAutoCopy: false,
        a7Value: '',
        a7AutoFormat: false,
        phoneAndEmailValue: '',
        phoneAndEmailAutoFormat: false,
        phoneAndEmailAutoCopy: false
    }
};
...
function validateAndCorrectConfig(loadedSettings: any): void {
    ...
    Object.keys(defaultConfig.preferences).forEach(key => {
        if (!(key in loadedSettings.preferences)) {
            loadedSettings.preferences[key] = defaultConfig.preferences[key];  // Adiciona a chave faltante
        }
    });
}
...
{   
        ...
        hWindow = new BrowserWindow({
            ...
            minWidth: 400,
            minHeight: 250,
            alwaysOnTop: settings.window.fixedWindow,
            frame: false,
            icon: path.join(__dirname, 'assets', 'otter.png'),
...
//Aqui temos uma sequencia de ipcMain onde ele aciona o comportamento para os botões do título da janela.
//Os que estiver 'main' são da página principal. Os que tiverem 'config' é da tela de configuração.
ipcMain.on('window-main-close', (event:any) => {
    app.quit();
})

ipcMain.on('window-main-minimize', (event:any) => {
    hWindow.minimize();
})

ipcMain.on('window-main-close-config', (event:any) => {
    properties_hWindow.destroy();
})

ipcMain.on('window-main-minimize-config', (event:any) => {
    properties_hWindow.minimize();
})

//Abre os changelogs em um navegador external ao programa.
ipcMain.on('open-url-on-external', (event:any, url:string) =>
{
    shell.openExternal(url);
})

//Executa a mensagem vindo do ipcRenderer onde define se a janela ficará fixada.
ipcMain.on('changed-main-pin', (event:any, isOnTop:boolean) =>
{
    if(isOnTop)
    {
        hWindow.setAlwaysOnTop(true);
    } else {
        hWindow.setAlwaysOnTop(false);
    }
    //Ele recarrega a janela.
    hWindow.reload();
})

//Habilitar redimensionamento da janela dinamicamente.
ipcMain.on('enable-resize-main-window', (event:any, isToResize:boolean) => {
    //Ele altera o parâmetro de redimensionável e recarrega a janela.
    hWindow.setResizable(isToResize);
    hWindow.reload();
})
